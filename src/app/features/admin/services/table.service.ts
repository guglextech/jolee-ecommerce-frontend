...
export class TableService {
  ...
  getPager(total_items: number, current_page: number, page_size: number) {
    ...
    let total_pages = Number(
      Math.ceil(Number(total_items) / Number(page_size))
    );
    ...
    if (Number(total_pages) <= Number(paginateRange)) {
      ...
      Number(current_page) <= Number(Math.floor(Number(paginateRange) / 2))
    ) {
      // Near the beginning
      start_page = 1;
      ...
      Number(current_page) >=
      Number(total_pages) - Number(Math.floor(Number(paginateRange) / 2))
    ) {
      // Near the end
    ...
    let end_index = Math.min(
      Number(start_index) + Number(page_size) - 1,
      Number(total_items) - 1
    );

    // create an array of pages to ng-repeat in the pager control
    let pages = Array.from(
      Array(Number(end_page) + 1 - Number(start_page)).keys()
    ).map((i) => Number(start_page) + Number(i));
