...
export class CartService {
  ...
  updateQuantity(value: number, item: OrderDetailsProduct) {
    if (value == -1) {
      ...
      if (item.quantity < item.total_quantity) {
        ...
        this.toast.error(
          `Cannot add more than ${item.total_quantity} items`,
          '',
          {
            timeOut: 80000000,
          }
  ...
  posAddToCart(item: OrderDetailsProduct) {
    const cartItem = this.posCartItems.find(
    ...
    if (totalQuantityInCart + item.quantity <= item.total_quantity) {
      ...
      this.toast.error(
        `Cannot add more than ${item.total_quantity} items in total.`,
        '',
        {
          timeOut: 80000000,
        }
  ...
  posRemoveCartItem(item: OrderDetailsProduct) {
    this.posCartItems = this.posCartItems.filter(
