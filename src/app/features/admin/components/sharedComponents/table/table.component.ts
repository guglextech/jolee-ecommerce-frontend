  ...
  onDateSelection(date: NgbDate) {
    if (!this.fromDate || (this.fromDate && this.toDate)) {
      ...
      if (date.after(this.fromDate)) {
        this.toDate = date;
        this.filter['date']['start_date'] = `${this.fromDate.year}-${String(
          this.fromDate.month
        ).padStart(2, '0')}-${String(this.fromDate.day).padStart(2, '0')}`;
        this.filter['date']['to_date'] = `${this.toDate.year}-${String(
          this.toDate.month
        ).padStart(2, '0')}-${String(this.toDate.day).padStart(2, '0')}`;
        this.selectedDate = `${this.filter['date']['start_date']} - ${this.filter['date']['to_date']}`;
      } else {
        this.fromDate = date;
      }
    ...
    if (this.filter['search'].trim() !== '') {
      filteredData = filteredData.filter((item: any) => {
        return Object.keys(item).some((key) => {
          ...
          if (
            typeof value === 'number' &&
            value.toString().includes(this.filter['search'])
          ) {
            return true;
          }
